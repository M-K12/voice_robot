GLOBAL_TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "hangup",
            "description": "仅在用户明确提出结束对话或挂断请求（如说'再见'/'挂断'/'退下'）时调用，严禁在回答普通问题后主动调用",
            "parameters": {
                "type": "object",
                "properties": {}
            }
...
def get_instructions(city: str) -> str:
    ...
    return (
        ...
        "5. 结束对话：仅当用户表达要再见、挂断、退下、去休息吧、退出、滚蛋、结束对话等明确告别意图时，才能使用 `hangup` 工具。严禁在解答完用户的常规问题（如天气查询等）后主动调用 `hangup`！\n"
        "当前默认城市是：" + city
    )
