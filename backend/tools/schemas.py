GLOBAL_TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "大屏图层调度工具。控制前端大屏切换、叠加展示各类图层要素。当用户明确要求看‘雷达分布图、卫星云图、台风路径图、视频监控、无人机画面、消防通道图层、积水内涝点图层等’可视化图层时，必须调用此工具。此工具仅影响大屏渲染，不返回天气实况数据。",
            "parameters": {
                "type": "object",
                "properties": {
                    "location_name": {
    ...
    },
    {
        ...
        "function": {
            "name": "hangup",
            "description": "仅在用户表达告别、结束对话或挂断请求（如说'再见'/'挂断'/'退下'/'退一下'/'退下吧'/'去休息吧'/'别说了'/'拜拜'）时调用，严禁在解答解答普通气象或应急问题后主动调用。",
            "parameters": {
                "type": "object",
                "properties": {}
            }
...
def get_instructions(city: str) -> str:
    ...
    return (
        ...
        "   - 当用户明确要求展示大屏图层，如雷达分布、卫星云图、台风路径、积水分布、视频监控、无人机画面、消防通道等图层时，必须调用 `show_screen_layer` 工具。在图层在大屏展示后，必须直接简短回复：“XX分布图已在大屏展示，请查看”（例如：“雷达分布图已在大屏展示，请查看”），严禁添加任何多余解释或长篇大论。\n"
        ...
        "5. 结束对话：仅当用户表达要再见、挂断、退下、退一下、退下吧、去休息吧、退出、滚蛋、结束对话等明确告别意图时，才能使用 `hangup` 工具。严禁在解答完用户的常规问题（如天气查询等）后主动调用 `hangup`！\n"
        "当前默认城市是：" + city
    )
